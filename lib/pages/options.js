		...
		this.get('#/:page(/:args)?', function()
		{
			...
			if ($('#' + this.params['page']).length)
			{
				...
				if (page == 'options' || page == 'about')
				{
					...
					{
						if (data != null && data['version'] > '0.6')
							Alert('error', 'There is a new version (' + data['version'] + ') of Untamed Now Playing available. If you\'re using the stable version, the extension will update automatically. However if you\'re using the development version, you need to <a href="' + data['update_url'] + '" target="_blank">update manually</a>');
					});
				}
...
function validateOptions()
{
	...
	plugin.saveBlobToFile($('#save_dir').val() + 'unp_write_test_53642784162133643354.txt', string2Bin('Just a quick test to ensure we can write to this directory! :)'));

	if (bin2String(plugin.contentsAtPath($('#save_dir').val() + 'unp_write_test_53642784162133643354.txt')) != 'Just a quick test to ensure we can write to this directory! :)')
	{
		Alert('error', 'Error: Unable to write to save directory');
		return false;
	}

	plugin.removeRecursively($('#save_dir').val() + 'unp_write_test_53642784162133643354.txt');
